...
from resumesh_llm.core.exceptions import ProviderError, RateLimitError, ConfigurationError
...
class MockClient(LLMClient):
    ...
    def __init__(self, model_name: str = "mock-model", mock_response: Optional[str] = None):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        if self.mock_response is not None:
            ...
            if request.response_format == "json_object":
                text_response = json.dumps({
                    ...
                    "highlights": ["Implemented mock provider client", "Configured robust tests"],
        ...
        return LLMResponse(
            text=text_response,
            usage=GenerationUsage(prompt_tokens=10, completion_tokens=15, total_tokens=25),
            provider="mock",
...
class OpenAIClient(LLMClient):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            raise ProviderError(str(e), provider="openai", status_code=e.status_code) from e
...
class GroqClient(LLMClient):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            raise ProviderError(str(e), provider="groq", status_code=e.status_code) from e
        except APIError as e:
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as http_client:
            try:
                ...
                raise ProviderError(f"HTTP request to Ollama failed: {str(e)}", provider="ollama") from e
